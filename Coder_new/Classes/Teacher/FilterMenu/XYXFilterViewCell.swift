...
class XYXFilterAnnexView: UIView {
    ...
    let clearBtn:UIButton = {
        ...
        button.setImage(UIImage.init(named: "Filter_Btn_Clear"), for: UIControl.State.normal)
        button.setTitleColor(UIColor(red: 240.0/255, green: 109.0/255, blue: 57.0/255, alpha: 1), for: UIControl.State.normal)
        button.setTitle("清空条件", for: UIControl.State.normal)
    ...
    override init(frame: CGRect) {
        ...
        clearBtn.addTarget(self, action: #selector(clearBtnClicked), for: UIControl.Event.touchUpInside)
        confirmBtn.addTarget(self, action: #selector(confirmBtnClicked), for: UIControl.Event.touchUpInside)
        addSubview(clearBtn)
